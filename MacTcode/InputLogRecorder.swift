...
final class InputLogRecorder {
    ...
    func start(in directory: URL) throws -> URL {
        ...
        let startedAt = Date()
        let url = nextSessionURL(in: directory, startedAt: startedAt)
    ...
    private func nextSessionURL(in directory: URL, startedAt: Date) -> URL {
        let timestamp = fileTimestamp(startedAt)
        let baseName = "keystrokes-\(timestamp)"
        var suffix = 0
        while true {
            let name = suffix == 0 ? baseName : "\(baseName)-\(suffix)"
            let url = directory.appendingPathComponent(name).appendingPathExtension("jsonl")
