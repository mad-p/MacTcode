...
MacTcodeには以下の入力モードがあります

* T-Codeモード (通常モード)
* 英数モード (直接モード)
    * T-Codeモードから `Ctrl-,` または `Ctrl-Shift-/` で切りかえます
    * `Ctrl-'` または `Ctrl-Shift-T` でT-Codeモードに戻ります
* 全角英数モード
    * `90` で入ります。英数字が全角文字になって入力されます
    * EnterキーまたはESCキーで戻ります
* 半角カナモード
    * `91` で入ります。カタカナ、句読点が半角文字になって入力されます
    * EnterキーまたはESCキーで戻ります
* 行入力モード
    * `88` で入ります。入力を未確定文字列にためてから、Enterキーで一気に入力します
    * 日本語IMEに対応していないアプリに入力したいときに便利です
