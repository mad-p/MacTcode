...
"keyBindings": {
  "actions": [
    { "keys": "hu", "action": "postfixBushu" },
    { "keys": "uh", "action": "postfixMazegaki", "inflection": false },
    { "keys": "58", "action": "postfixMazegaki", "inflection": true },
    { "keys": "19", "action": "selfInsertAndDirectMode", "text": "19" },
    { "keys": "20", "action": "selfInsertAndDirectMode", "text": "20" },
    { "keys": "90", "action": "zenkakuMode" },
    { "keys": "88", "action": "lineMode" },
    { "keys": "\\", "action": "symbolSet1" },
...
- **`actions`**: アクションのキー割り当て。各要素は `keys`（キーシーケンス）と `action`（下表のアクション識別子）を指定します。空配列 `[]` を指定すると、アクションの既定割り当てをすべて無効にできます
    - **`postfixBushu`**: 部首変換
    - **`postfixMazegaki`**: 交ぜ書き変換。`inflection: true` を指定すると活用変換になります（未指定時は `false`）
    - **`zenkakuMode`**, **`zenkakuOneMode`**, **`hankanaMode`**: 全角、1文字全角、半角カナ入力モード
...
- キーシーケンス `keys` の設定値について
    - 入力される文字を指定してください。上記の `hu`、`uh` はDvorak配列の場合の例となります
    - バックスラッシュやダブルクォートはエスケープしてください
    - `C-,` の形式でコントロール+記号(`',.-=/;`)が利用できます(1文字に限ります)
    - 同じ `keys` を複数の `actions` に指定した場合、後の定義は無効になりログへ記録されます。未知のアクションや必須引数がない定義も無効になります
- 旧形式の `bushuConversion` などの個別フィールドは、`actions` が未指定の場合に限り互換目的で引き続き使用できます。新規設定では `actions` を使用してください
...
- **`streamStatsEnabled`**: 漢直ストリーム統計の記録有効/無効
  ...
  - `strokeStatsEnabled`が`false`の場合、ストリーム統計も記録されません
